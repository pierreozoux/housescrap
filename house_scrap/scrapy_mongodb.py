...
class MongoDBPipeline():
    ...
    def configure(self):
        ...
        options = [
            ...
            ('unique_key_exit', 'MONGODB_UNIQUE_KEY_EXIT'),
    ...
    def insert_item(self, item, spider):
        ...
        if self.config['unique_key'] is None:
            ...
            log.msg("Saving house in the database", level=log.INFO)
            if self.config['unique_key_exit']:
                try:
                    self.collection.insert(item, continue_on_error=True)
                except errors.DuplicateKeyError:
                    log.msg("House already in the database", level=log.INFO)
